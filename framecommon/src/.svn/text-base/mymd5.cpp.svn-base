/*-------------------------------------------------------------------/
         Name        :         md5.cpp
         Purpose        :        calculate the md5
          Author        :        Kelvin U.V
         Date        :        2002/10/01 4:43
    Email   :   jruv@163.com
/-------------------------------------------------------------------*/
...
if(nErr==0)
{
    if(strlen(fname) > 220)fname[220]='\0';
#ifndef ENGLISH
        //sprintf(strErr,"打不开文件 [ %s ] !\n请确认文件是否存在 !",fname);
        snprintf(strErr, sizeof(strErr), "打不开文件 [ %s ] !\n请确认文件是否存在 !",fname);
#else
        //sprintf(strErr,"Can not open the file [ %s ] !",fname);
		snprintf(strErr, sizeof(strErr), "Can not open the file [ %s ] !",fname);
#endif
}
else 
{
		...
		strncpy(strErr, ErrInfo[nErr][1], sizeof(strErr));
#else 
        //strcpy(strErr,ErrInfo[nErr][0]);
		strncpy(strErr, ErrInfo[nErr][0], sizeof(strErr));
#endif
}
}
LPCSTR CMd5::GetErr()
...
void  CMd5::JustifytoStr()
{
        ...
        for(int i=0;i<4;i++)
        {
                ...
                //sprintf(temp,"%08lx",T[i]);
                snprintf(temp,sizeof(temp),"%08lx",T[i]);
                strcat(Digest,temp);
        }
}

LPCSTR CMd5::GetMd5Buf()
{    
    return (LPCSTR)T;
}
}
